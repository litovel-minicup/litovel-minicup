<template>
    <div>
        <transition name="fade-block">
            <div class="Aside" v-if="stories.length">
                <div class="Box__head Aside__head">
                    <div class="Aside__head__icon Box__head__icon">
                        <svg width="75" height="75" version="1.1"
                            ...
                            <g
                                    id="g1942"
                                    transform="matrix(0.18575583,0,0,0.18575583,13.723254,13.723254)"
                    ...
                    <div class="Stories">
                        <div class="Stories__story" v-for="(story, i) in stories" @click="selected = story">
                            <template v-if="story.type === 'video'">
                                ...
                                <span class="Stories__taken">{{ story.taken_at | relativeTaken }}</span>
                            </template>
        ...
        <transition name="fade-detail">
            <div class="Stories__backdrop" v-if="selected !== null" @click="selected = null">
                <div class="Stories__detail">
                    <template v-if="selected.type === 'video'">
                        <video :src="selected.url + '#0.1'" preload="metadata"
                               controls autoplay @click.stop @ended="next"></video>
                    </template>
                    <template v-if="selected.type === 'image'">
                        <div class="Stories__progress"></div>
...
<script>
    ...
    const timeAgo = new TimeAgo('cs');


    export default {
        name: "App",
        data() {
            return {selected: null, timeout: null}
        },
        computed: {
            ...mapState({
                stories: (state) => _.reverse(_.sortBy(state.stories, 'taken_at'))
            }),
        },
        methods: {
            ...mapActions(['loadStories']),
            next() {
                let i = _.findIndex(this.stories, this.selected) + 1;
        ...
        watch: {
            selected(new_, old) {
                ...
                if (new_.type === 'image') this.timeout = setTimeout(this.next, 8000)
            }
        }
    }
</script>
